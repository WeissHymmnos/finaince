...
## 会话七：全量入库 + P2 清欠 + 覆盖率门禁 + 批内并行

**Git 收口（用户指令：删除提交记录中一切 AI 辅助工具痕迹 + 分批原子 commit）**
- 历史净化：唯一含 AI 署名的 HEAD 提交 bd358ce（"Generated by Mistral Vibe" + Co-Authored-By）amend 为 b9b3068；全 32 提交经 AST 级消息扫描确认零 AI 工具痕迹；作者/提交者身份全部为用户本人。重写后需 force-push 同步远端。
- 会话一~六全部未提交工作按依赖序分 8 个原子 commit 入库：scoring 单一数学源 → AST 门禁 → eval 缓存/jobs 并发闸 → bench 数字轨 → bwrap 沙箱 → agent 族（进化/组合/BRAIN/campaign/记忆）→ desk 面+SPA → 文档与工具链。

**P2 审计清欠**
- 鉴权防呆：serve 的 21 处手写 `_require_desk(request)` 收敛为 ASGI 中间件 `DeskAuthGate` 默认拒绝——`/api/v1/*` 全部默认要求 token，仅 health/baseline 白名单放行，未来新增路由不可能因遗漏而裸奔；OPTIONS 放行 CORS 预检，aiminer `/api/*` 与 SPA 维持自身契约。（中间曾尝试 app 级 FastAPI dependency，发现其作用于 /ws 且该 FastAPI 版本对 WS 路由不绑定 Request 参数致 TypeError，遂改 ASGI 中间件方案。）
- SQLite 迁移统一：新增 `db.ensure_columns(conn, table, columns)`，trace/jobs/audit/catalog 四处 PRAGMA+ALTER 手写块收敛；audit 此前 ALTER 从不 commit 的缺陷随之消除（助手内即时持久化）。
- LLM key 映射收敛：供应商→env 键映射单点化到 `runtime.PROVIDER_ENV_KEYS`；settings.inject_aiminer_api_key 删除私有副本，runtime.inject_llm_env 的 deepseek 硬编码分支改为通用查表；两注入器的 overwrite/setdefault 语义差异保留（有意设计）。
- aiminer_fallback 清理：删除 `/api/charts/{id}` 与 `/api/strategies/{id}/charts/{kind}`——全仓无任何代码写入 chart_path(s)，两路由成功路径不可达且 stub SPA 从不调用；其余 fallback 处理器均为可达的诚实降级应答，保留。
- 静默异常审计：AST 扫出 88 处宽捕获无 raise/emit。分诊后仅修「吞掉会污染研究结论」的一类：homogeneous 门禁语料加载（discovery/gates）、expr_hash 计算（store）、advisor 上下文注入（loop）、rebuild 导入/重打标、DSR 试验计数（gates）现均 emit obs 事件或返回计数字段；探针类/遥测套遥测类/已落 error 字段类保持静默为设计。

**测试与门禁**
- 新增 tests/test_branch_coverage.py：brain_track HTTP 五分支（拒绝/缺 Location/成功取级/poll 失败/超时零轮询）、code_evolution 重写恢复轮（edit_motive 断言）与门禁失败不晋级分支、db.ensure_columns 加列-持久化-幂等三语义、DeskAuthGate 默认拒绝/白名单/预检契约。
- pytest-cov 进入 [dev] extra；九个治理模块（expr_ast/db/data_track/combination/code_evolution/corpus_campaign/brain_track/process_memory/domain.scoring）实测覆盖 84%，pyproject `[tool.coverage.report] fail_under=82` 生效，offline CI 增加 coverage pass。
- WS-C 后置项落地：`run_loop(workers=N)`——多表达式队列经 spawn 上下文 ProcessPoolExecutor 批内并行预评估，队列序消费保持 trace 确定性；worker 级失败成为诚实 failed step 不毒化整批；池不可用回退串行。贯通 run_loop_job / POST /api/v1/loop（clamp 1..8）/ `finaince loop --workers`。spawn 非 fork 是有意选择：父进程持 sqlite 锁时 fork 子进程会死锁。

**验证**：pytest 269 passed / 2 skipped；ruff 全绿；测试基线 259→269。
