...
def test_workbench_mutations_round_trip(isolated_home: Path) -> None:
    ...
    validated = client.post("/api/backtest/validate", json={"expression": "Rank($close)"})
    ...
    got = client.get(f"/api/backtest/{job_id}")
    assert got.status_code == 200
    history = client.get("/api/backtest/history")
    ...
    assert any(item.get("strategy_id") == strategy_id for item in client.get("/api/strategy/history").json())

    saved = client.put("/api/wiki/page/desk_note", json={"content": "---\ntitle: Desk note\n---\nupdated\n"})
    assert saved.status_code == 200
    assert saved.json()["status"] == "saved"
    page = client.get("/api/wiki/page/desk_note")
    assert page.status_code == 200
    assert "updated" in page.text

    preview = client.post("/api/admin/reset", json={"scopes": ["runs"], "confirm": False})
    ...
    assert client.delete(f"/api/backtest/{job_id}").json()["status"] == "deleted"
    assert client.delete(f"/api/strategy/{strategy_id}").json()["status"] == "deleted"
    assert client.get(f"/api/backtest/{job_id}").status_code == 404
    assert client.delete("/api/swarm/runs/missing").status_code == 404
